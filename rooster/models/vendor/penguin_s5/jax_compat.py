...
#         src/models/layers/S5_jax_compat.py
...
from typing import Any, Callable, Iterable, List, Literal, Optional, Sequence, TypeVar, overload
...
def _scan(tree, operator, elems, axis: int):
    ...
    if num_elems % 2 == 0:
        even_elems = combine(
            tree, operator, [torch.ops.aten.slice(e, axis, 0, -1) for e in odd_elems], [torch.ops.aten.slice(e, axis, 2, None, 2) for e in elems]
        )
    else:
        even_elems = combine(tree, operator, odd_elems, [torch.ops.aten.slice(e, axis, 2, None, 2) for e in elems])
    ...
    even_elems = [
        ...
        else torch.ops.aten.slice(elem, axis, 0, 1)  # Jax allows/ignores concat with 0-dim, Pytorch does not
...
def test_associative_scan(shape=(1, 24, 24)):
    ...
    assert np.isclose(ty1.numpy(), np.array(jy1)).all() and np.isclose(ty2.numpy(), np.array(jy2)).all(), (
...
def _complex_truncated_normal(upper: float, shape: Sequence[int], dtype, device=None) -> torch.Tensor:
    """
    ...
    t = (1 - torch.exp(torch.tensor(-(upper**2), dtype=dtype, device=device))) * torch.rand(shape, dtype=real_dtype, device=device).type(dtype)
...
def test_variance_scaling():
    ...
    print(tn_c32.shape)
    assert np.isclose(tn_c32.std().item(), expected_std, rtol=0.015, atol=0.015), (
